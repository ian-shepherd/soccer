...
excluded_urls = pd.read_csv(inputFolder + 'excluded_match_urls.csv')
match_dim = pd.read_csv(dbFolder + 'match_dim.csv')

#Filter excluded matches
match_urls = match_urls.merge(excluded_urls, how='left', on='urls')
match_urls = match_urls[match_urls['reason'].isnull()].drop(columns={'reason'})
...
with open(outputFolder + 'raw_match_data.pickle', 'wb') as filename:
    pickle.dump(matches, filename)

with open(outputFolder + 'raw_errors_match_data.pickle', 'wb') as filename:
