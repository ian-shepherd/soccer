...
inputFolder = rootFolder + '/Data/Flat/'
outputFolder = rootFolder + '/Data/Update/'

# Load data
match_urls = [
    '/en/matches/b80c1dd8/Arminia-Bayer-Leverkusen-November-21-2020-Bundesliga',
    '/en/matches/a58f05b9/Newcastle-United-Manchester-United-February-11-2018-Premier-League',
    '/en/matches/47b76f62/Chelsea-West-Bromwich-Albion-February-12-2018-Premier-League',
    '/en/matches/f35f4268/Huddersfield-Town-Swansea-City-March-10-2018-Premier-League',
    '/en/matches/5e35e444/Bournemouth-Manchester-City-March-2-2019-Premier-League',
    '/en/matches/18ccfdb9/Sheffield-United-Newcastle-United-January-12-2021-Premier-League',
    '/en/matches/55653337/Huesca-Real-Betis-January-5-2019-La-Liga',
    '/en/matches/7011948c/Leganes-Espanyol-May-12-2019-La-Liga',
    '/en/matches/69d02fd4/Atletico-Madrid-Villarreal-February-23-2020-La-Liga',
    '/en/matches/a0933679/Valladolid-Celta-Vigo-June-17-2020-La-Liga',
    '/en/matches/e61d95c0/Real-Betis-Alaves-July-16-2020-La-Liga',
    '/en/matches/01b7d3d4/Lyon-Nimes-September-18-2020-Ligue-1',
    '/en/matches/93085bed/Internazionale-Cagliari-April-17-2018-Serie-A',
    '/en/matches/804e3527/Napoli-Atalanta-October-17-2020-Serie-A',
    '/en/matches/f68d48b6/Internazionale-Benevento-January-30-2021-Serie-A',
    '/en/matches/241e3375/Club-Brugge-Dortmund-September-18-2018-Champions-League',
    '/en/matches/f7147222/Napoli-Liverpool-October-3-2018-Champions-League',
    '/en/matches/8c22ccee/Dinamo-Zagreb-Spartak-Trnava-November-8-2018-Europa-League',
    '/en/matches/c339d57c/Espanyol-Ludogorets-Razgrad-November-7-2019-Europa-League',
    '/en/matches/6faf2661/Tottenham-Hotspur-Ludogorets-Razgrad-November-26-2020-Europa-League',
    '/en/matches/52acd3d4/Real-Salt-Lake-Toronto-FC-May-18-2019-Major-League-Soccer',
    '/en/matches/40b3eed1/New-York-City-FC-FC-Cincinnati-June-6-2019-Major-League-Soccer',
    '/en/matches/dfe3f4a0/DC-United-New-York-City-FC-September-6-2020-Major-League-Soccer',
    '/en/matches/2a5361bd/Real-Salt-Lake-Los-Angeles-FC-October-4-2020-Major-League-Soccer'    
    ]
...
def events(pageSoup, side):
    
    summary = pageSoup.find('div', {'id' : 'events_wrap'})
    
    
    if side == 'x':
        events = summary.find_all('div', {'class' : 'event a'})
    else:
        events = summary.find_all('div', {'class' : 'event b'})
...
def shots(commentsSoup):
    
    
    shots = commentsSoup.find('tbody')
    shots = shots.find_all('tr')
    
    shotList = []
    
    for row in range(0,len(shots)):
        
        mydict = {}
    
        mins = shots[row].find('th', {'data-stat' : 'minute'}).text
        if mins == "":
            continue
        td = shots[row].find_all('td')
        
        player = td[0].find('a', href=True)['href']
        outcome = td[2].text
        distance = td[3].text
        body_part = td[4].text
        notes = td[5].text
        try:
            sca_player1 = td[6].find('a', href=True)['href']
            sca_player1_event = td[7].text
        except:
            sca_player1 = None
            sca_player1_event = None
            
        try:
            sca_player2 = td[8].find('a', href=True)['href']
            sca_player2_event = td[9].text
        except:
            sca_player2 = None
            sca_player2_event = None
        
        mydict['minute'] = mins
        mydict['player'] = player
...
def match_data(url):
    ...
    match_events_x = events(pageSoup, 'x')
    match_events_y = events(pageSoup, 'y')
    ...
    try:
        shots_x = shots(BeautifulSoup(comments[26], 'html.parser'))
    except:
        shots_x = []
    try:
        shots_y = shots(BeautifulSoup(comments[27], 'html.parser'))
    except:
        shots_y = []
...
for i in range(0,len(match_urls)):
    ...
    try:
        match = match_data(match_urls[i])
        matches.append(match)
        
        print(i, 'done')
        time.sleep(random.randint(3,9))
    except:
        errors.append(match_urls[i])
        print(i, 'ERROR')
        time.sleep(random.randint(3,9))


# with open(outputFolder + 'raw_match_data_errors_fixed.pickle', 'wb') as filename:
#     pickle.dump(matches, filename)

# with open(outputFolder + 'raw_errors_match_data2.pickle', 'wb') as filename:
